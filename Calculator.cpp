...
Calculator::Calculator(int numJumps, double target)
	: NUM_JUMPS(numJumps), TARGET(target)
{
	...
	_snakeSize_distribution = uniform_int_distribution<int>(-300, -200);
}
...
double Calculator::Expectation(const Jumps & jumps)
{
	...
	while (cumul_prob < 0.999999999)
	{
		...
		if (counter > 10000 || expectation > TARGET + _tolerance)
			return 0.0;
	}
...
Jumps Calculator::RandomJumps2()
{
	...
	int num_ladders = _ladderSize_distribution(_generator);
...
Jumps Calculator::SearchJumps(double tolerance)
{
	...
	do
	{
		...
		//	cout << " Error = " << setw(10) << bestError;
		//	cout << " Exp.  = " << setw(10) << bestExp;
		//	cout << endl;
		//}
		if (error < 1E-3)
		{
